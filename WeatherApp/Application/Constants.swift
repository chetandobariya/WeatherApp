...
struct Constants {
    ...
    struct AppURL {
        static var baseUrl: URL {
            guard let url = URL(string: "https://api.openweathermap.org/data/3.0") else {
                fatalError("baseUrl not found")
            }
            return url
        }
        static var geoURL: URL {
            guard let url = URL(string: "https://api.openweathermap.org/geo/1.0") else {
                fatalError("baseUrl not found")
            }
            return url
        }
